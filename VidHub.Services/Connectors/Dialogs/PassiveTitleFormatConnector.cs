...
{
    public class PassiveTitleFormatConnector(IVideoService vs, IVidHubSettings settings) : ConnectorTemplate(vs), ITitleFormatConnector
    {
        ...
        {
            logger.LogTrace("UpdateTitles invoked (passive)");
            try
            {
            ...
            {
                InvalidRegex = true;
                logger.LogWarning(ex, "Invalid regex pattern in PassiveTitleFormatConnector");
            }
            ...
            logger.LogDebug("Passive UpdateTitles completed for {Count} videos", videos.Count);
        }
    }
}
