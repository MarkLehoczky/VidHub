namespace VidHub.Core.Streams
{
    ...
    {
        public int SampleRate { get; set; } = metadata.TryGetValue("sample_rate", out string? value) && int.TryParse(value, out int result)
            ? result
            : 0;
        public int ChannelCount { get; set; } = metadata.TryGetValue("channels", out string? value) && int.TryParse(value, out int result)
            ? result
            : 0;
        public string ChannelLayout { get; set; } = metadata.TryGetValue("channel_layout", out string? value)
            ? value
            : string.Empty;
        ...
        public AudioStream() : this(new Dictionary<string, string>()) { }
    }
}
