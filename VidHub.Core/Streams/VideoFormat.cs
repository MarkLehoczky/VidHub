namespace VidHub.Core.Streams
{
    public class VideoFormat(IDictionary<string, string> metadata) : MetadataSource(metadata)
    {
        ...
        public TimeSpan Duration { get; set; } = metadata.TryGetValue("duration", out string? value) && double.TryParse(value, out double durationSeconds)
                    ...
                    : TimeSpan.Zero;
        public string Format { get; set; } = metadata.TryGetValue("format_long_name", out string? value) ? value : string.Empty;
        public long Size { get; set; } = metadata.TryGetValue("size", out string? value) && long.TryParse(value, out long result) ? result : 0;
